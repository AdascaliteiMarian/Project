...
const user = require("../db/user-db/user");

/* Render Login Page */
router.get("/", function (req, res) {
  if (req.session.wrong_login == false) {
    req.session.wrong_login = "Username or password incorrect";
...
router.post("/", function (req, res) {
  let login_req_result = 0;
  let l_username = req.body.l_username;
  username = l_username;
  let l_password = req.body.l_password;
  login_req_result = user.searchForUser(l_username, l_password);
  login_req_result.then((result) => {
    if (result.length == 1) {
...
router.post("/add-user-calories", function (req, res) {
  ...
  let userinfo = user.getInfo();
  let food_name = req.body.example;
  let food_calories = user.getSelectedCalories(food_name);
  food_calories.then((result) => {
    if (req.session.count == 0) {
      req.session.calories_counted = result;
      ++req.session.count;
      req.session.all_food_names =
        req.session.all_food_names + "  " + food_name;
    } else {
      req.session.all_food_names =
        req.session.all_food_names + " + " + food_name;
...
router.post("/search-user-calories", function (req, res) {
  let calendar_date = req.body.calendar_date;
  let pass_calories = user.showCaloriesOnDate(
    calendar_date,
    req.session.username
  );
